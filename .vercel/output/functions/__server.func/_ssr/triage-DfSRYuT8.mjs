...
function computeTriage(answers) {
  ...
  if (red) {
    return {
      status: "red",
      reasons,
      message: "This may be urgent. Call the owner and the avian vet now. Keep the bird warm, quiet, and minimize handling."
    };
  }
  if (yellowCount >= 2) {
    return {
      status: "yellow",
      reasons,
      message: "Two or more concerning signs together — this combination is worth a call. Message the owner now and call the avian vet if it continues or worsens."
    };
  }
  if (yellowCount === 1) {
    return {
      status: "yellow",
      reasons,
      message: "Something may be off. Message the owner, take photos if helpful, and keep monitoring. Call the avian vet if this continues or appears with other signs."
    };
  }
  return {
    status: "green",
    reasons: [],
    message: "Nothing concerning logged. Keep following the care plan and watching for any changes."
  };
}
const TRIAGE_DISCLAIMER = "This app does not diagnose illness and is not a substitute for veterinary care. Birds can decline quickly. If something seems off, contact the owner and an avian veterinarian.";
export {
  SCAN_FIELDS as S,
  TRIAGE_DISCLAIMER as T,
  computeTriage as c
};
