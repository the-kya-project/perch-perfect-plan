  ...
  const { data: bird } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data } = await supabase
        ...
        .maybeSingle();
      return {
        row: data as { id: string; name: string; owner_id: string } | null,
        uid: u.user?.id ?? null,
      };
    },
  });
    ...
    <div className="min-h-screen bg-[#f4f1e8] pb-[calc(var(--nav-spacer)+6rem)]">
      <header className="sticky top-0 z-10 border-b border-[#e3ded0] bg-[#f4f1e8]/95 backdrop-blur">
        <div className="mx-auto flex max-w-md items-center gap-3 px-5 pt-safe pb-3">
          <Link
            ...
            params={{ birdId }}
            aria-label="Back"
            className="-ml-1 rounded p-1 text-[#1a3d2e]"
          >
      ...
      <main className="mx-auto max-w-md space-y-4 px-5 py-5">
        <p className="text-sm leading-relaxed text-[#5f5e5a]">
          Pass {name}'s record to the adopter so they have everything you've learned. The record
          goes with the bird; once accepted, you won't have access anymore.
        </p>
        ...
        <div
          ...
          style={{ background: "#FCEBEB", border: "1px solid #E24B4A" }}
        >
          <p className="flex items-start gap-2 text-sm" style={{ color: "#791F1F" }}>
            ...
            <span>
              <span className="font-medium">This can't be undone.</span> Once they accept, the
              record is theirs. You won't be able to view or edit it.
            </span>
          </p>
        </div>
      </main>
      ...
      <footer className="fixed inset-x-0 bottom-[var(--nav-spacer)] border-t border-[#e3ded0] bg-[#f4f1e8] px-5 py-3">
        <div className="mx-auto max-w-md space-y-2">
          <button
            type="button"
            disabled={!emailOk || m.isPending}
            onClick={() => m.mutate()}
            className="flex min-h-[48px] w-full items-center justify-center gap-2 rounded-[14px] bg-[#1a3d2e] text-sm font-medium text-white disabled:opacity-50"
          >
            <ArrowRightLeft className="size-4" /> {m.isPending ? "Sending…" : "Send handoff"}
          </button>
          <button
            type="button"
            onClick={() => navigate({ to: "/birds/$birdId/export", params: { birdId } })}
            className="block w-full py-1 text-center text-xs font-medium text-[#5f5e5a] underline"
          >
