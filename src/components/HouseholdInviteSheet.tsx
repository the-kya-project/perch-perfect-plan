  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col bg-[#f4f1e8]">
      <header className="flex items-center justify-between border-b border-[#e3ded0] px-5 py-3 pt-[max(env(safe-area-inset-top),0.75rem)]">
        <button type="button" onClick={onClose} className="text-sm font-medium text-[#5f5e5a]">
          Cancel
        </button>
      ...
      <div className="mx-auto w-full max-w-md flex-1 space-y-4 overflow-y-auto px-5 py-5">
        ...
        {targetBirdId && (accountLoading || eligible.length > 0) && (
          <div>
            <span className="mb-1 block text-[11px] font-semibold uppercase tracking-wider text-[#5f5e5a]">
              Already in your household
            </span>
            <div className="overflow-hidden rounded-[14px] bg-white ring-1 ring-[#e3dcc9]">
              {accountLoading ? (
                <div className="flex items-center gap-2 px-4 py-4 text-sm text-[#8a897f]">
                ...
                eligible.map((m, i) => {
                  ...
                  return (
                    <div
                      key={m.userId}
                      className={`flex min-h-[56px] items-center gap-3 px-4 ${i ? "border-t border-[#ece6d6]" : ""}`}
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        {m.email && m.name && (
                          <p className="truncate text-xs text-[#8a897f]">{m.email}</p>
                        )}
                      </div>
                      <button
                        type="button"
            ...
            <div className="mt-4 flex items-center gap-3">
              ...
              <span className="text-[11px] font-medium uppercase tracking-wider text-[#8a897f]">
        ...
        <p className="text-sm text-[#5f5e5a]">
          They'll get an email invite to create an account and help care for your birds.
        </p>
        ...
        <div>
          <span className="mb-1 block text-[11px] font-semibold uppercase tracking-wider text-[#5f5e5a]">
            Which birds can they access?
          </span>
          <div className="overflow-hidden rounded-[14px] bg-white ring-1 ring-[#e3dcc9]">
            {birds.map((b, i) => (
              <label
                key={b.id}
                className={`flex min-h-[48px] cursor-pointer items-center gap-3 px-4 ${i ? "border-t border-[#ece6d6]" : ""}`}
              >
                <input
                  type="checkbox"
                  checked={selected.has(b.id)}
                  onChange={() => toggle(b.id)}
                  className="size-4 accent-[#1a3d2e]"
                />
        ...
        <div>
          ...
          <div className="overflow-hidden rounded-[14px] bg-white ring-1 ring-[#e3dcc9]">
            {ASSIGNABLE_PRESETS.map((p, i) => (
              <label
                key={p}
                className={`flex min-h-[56px] cursor-pointer items-start gap-3 px-4 py-3 ${i ? "border-t border-[#ece6d6]" : ""}`}
              >
                <input
                  type="radio"
                  name="preset"
                  ...
                  className="mt-0.5 size-4 shrink-0 accent-[#1a3d2e]"
                />
                <span className="min-w-0">
                  <span className="block text-sm font-medium text-[#1a3d2e]">
                    {PRESET_LABELS[p]}
                  </span>
          ...
          <p className="mt-1.5 px-1 text-[11px] text-[#8a897f]">
            You can fine-tune this anytime on the permissions screen.
          </p>
        </div>
      ...
      <footer className="border-t border-[#e3ded0] px-5 py-3 pb-[max(env(safe-area-inset-bottom),0.75rem)]">
        <div className="mx-auto flex max-w-md gap-2">
          <button
            type="button"
            onClick={onClose}
            className="min-h-[44px] flex-1 rounded-[14px] border border-[#c8bfa6] text-sm font-medium text-[#1a3d2e]"
          >
            Cancel
          </button>
          <button
            ...
            disabled={!canSend}
            onClick={() => m.mutate()}
            className="min-h-[44px] flex-1 rounded-[14px] bg-[#1a3d2e] text-sm font-medium text-white disabled:opacity-50"
          >
